...
int main()
{
	...
	int _N[7];						//定义每个电极之间所取的网格数（水平方向），加下划线为了与全局变量N区别
	double V[6];					//定义各个电极电压
	...
	//计算总网格数M与N
	M = M1 + M2;
	N = _N[0] + _N[1] + _N[2] + _N[3] + _N[4] + _N[5] +_N[6]+ 2 * n; 
		...
		for (int j = 0; j < N; j++)
		{
			...
			else	
			{
				...
				while(k < n)
				{
					if (j <= sum)
					{
						if (i == 0)  //如果i=0直接做封闭边界处理
						{
							grid[i][j].is_margin = true;
							if (k == 0)  //若在阴极和第一个电极间，线性插值计算边界电压
							{
								grid[i][j].voltage = (V[0] - cathode_voltage) * j / (_N[0] + 1);
							}
							else if (k != (n - 1))  //若不在最后一个电极到荧光屏区间
							{
								grid[i][j].voltage = V[k - 1] + (V[k] - V[k - 1]) * (_N[k] + j - sum) / (_N[k] + 1);
							}
							else  //最后一个电极到荧光屏区间
							{
								grid[i][j].voltage = V[k - 1] + (screen_voltage - V[k - 1]) * (_N[k] + j - sum) / (_N[k] + 1);
							}
						}
